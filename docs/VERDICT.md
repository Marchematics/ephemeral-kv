...
| C11 | what would widen the quality-admissible routing region | The state is now admissible at **4,608 tokens** (0.00 pp, window 3,584), which is the size the replay's **4,096** column covers - and that column advances **63 of 198** cells against the 8,192 column's **52 of 294**, so regenerating the join at the new floor would take the admissible region to **117 cells at a smaller state** (the replay at 4,608 is queued).  Before this measurement the best 4,096-token state was **-2.40 pp**, 0.4 pp outside the allowance, with the deficit in the far field rather than the window (the same 3,072-token window holds 0.00 pp once the total is 6,144).  So a 4,096-token state inside the allowance would widen the region from **52 to 115 cells at half the state size**, with capacity and mobility improving with it - the state halving and the region widening are the same measurement.  The stricter bar, decision parity with plain retrieval, needs 0.237-0.243 F1 against the 0.161-0.179 that the fidelity-admissible arms reach: a gap of 0.06-0.08 that no arm we have measured closes | `g2-compiler-window3k-b{4096,6144}-v1.json`, `g4-quality-join-v2.json`.  The 4,096-token admissibility is a *target*: it is the run the queue answers, and it is stated as a target rather than a result |
