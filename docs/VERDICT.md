...
| C4b | the *de facto* baseline - model-written compaction - at the same budget and the same window | **At fidelity parity: 0.00 pp** (NLL -0.020, 223 summariser calls / 395 reuses over 48 examples, view 6,650 tokens p50) with a 6,656-token verbatim window and a 512-token model-written summary of the older history.  The comparison that matters is therefore *what the remaining ~1.5K buys*: the summary or retrieved evidence - the matched-window arm is queued and the decision half is not yet measured.  An earlier version of this row reported -23.86 pp and was withdrawn: the arm never called its summariser (a branch-placement bug left a malformed recency variant), which is why the receipts now record `summariser_calls` and the audit asserts it | scope: one summariser (the served 1B model), one split (6,656 + 512), 48 examples |
