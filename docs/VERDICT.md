...
| C7 | the cost law moves routing at a quality-admissible state, including a hotspot | **134/624** replay cells advance, **117 of them at a fidelity-admissible state**: **63 at 4,096** (the measured floor, -0.96 pp with a 3,584-token window), 52 at 8,192 (0.00 pp) and 2 at 16,384 (-1.94 pp, retrieval-only); a hotspot variant (one worker at a tenth of the service rate) advances in all three regimes (16/48 cells).  The magnitudes behind those 52: all clear the **1.5x SLO-goodput** bar that defines an advance and none needs the p99 route (best p99 improvement **26%**, under the 30% bar).  Twenty-six clear it with a finite ratio (median **1.72x**, max **5.06x**, balanced median **2.15x**) and in the other twenty-six the strongest baseline completes *no* work, so the ratio is unbounded rather than large; by regime, balanced 10, slow-worker 13, worker-loss 29.  It is a throughput win: where the baseline stalls, the ephemeral policy's own p99 is worse in absolute terms | the balanced, hotspot and slow-worker wins are the no-cluster-KV-store regime with a warm cache holding a fraction of the fleet.  **And the decision at that state is a statistical tie with retrieval, not a point-estimate win**: the joint arm scores 0.161 against plain retrieval's 0.237 floor, so under a strict point-estimate parity bar **0** cells clear it, while the paired interval (+0.081, CI [-0.016, +0.177]) includes zero.  A phase-diagram cell can be called quality-admissible for the surface, or for the decision against full history (132 cells), but not for beating retrieval |
