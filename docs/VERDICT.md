...
| C4b | (withdrawn; being re-measured) model-written compaction as the baseline | **Withdrawn.**  A first measurement reported -23.86 pp of fidelity, but a direct probe showed the arm never called its summariser: a branch-placement bug left the "compaction" view a malformed recency variant with no summary text in it.  The bug is fixed (branch hoisted out of the consolidation gate, the window respects the same budget as every other arm, the summary is counted in the reported state, and `summariser_calls` is recorded per row so the claim is checkable), the two artifacts are deleted, and the corrected numbers will be recorded here when the re-runs finish.  Until then the paper makes no compaction claim | scope: none yet - there is no valid receipt |
