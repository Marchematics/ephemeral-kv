...
| C2b | at a fixed budget the window and the decision trade off, measured along the window axis | 6,656-token window + ~1.5K retrieval: fidelity **0.00 pp**, decision **0.089** (the full-history level); 4,096-token window + ~4.1K retrieval: fidelity **0.00 pp**, decision **0.161**; no window, 4,096 retrieval: fidelity -25.00 pp, decision 0.243.  Both windowed rows hold the surface; the difference of 0.072 F1 is what the window's claim on the budget costs the decision | 48 paired sessions; two window points, so the curve is bracketed rather than traced |
