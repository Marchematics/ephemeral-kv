...
| cluster consequence >=1.5x goodput or >=30% p99 in several regimes | two or more regimes | both routes now clear, in different regimes: capacity-pressure and worker-loss cells advance on goodput (median 1.61x, max 5.06x over 117 cells) while the flash crowd advances on the tail (23 cells at >=30% p99, best +68.7%); balanced with all sessions resident and a KV-moving tier still does not advance | **met, both routes** |
