...
| the changed cost law moves the routing decision at a quality-admissible state | **174/724** replay cells advance; **157 of them at a state whose measured quality point passes** - 77 at the 4,096-token floor, 18 at 6,144, 59 at 8,192, 3 at 16,384 - across balanced, slow-worker, worker-loss, a flash crowd and a heavy-tailed size mix (`g4-quality-join-v3.json`) |
...
| real agent task not regressing, ideally > raw retrieval | no regression | on file localisation all arms beat full history (0.089 -> 0.139-0.243) and the shipped view ties retrieval (0.165 against 0.169, paired +0.013, CI [-0.068, +0.092]); against raw retrieval the *compiler* loses.  **On the executable form - 48 mid-session turns parsed into tool invocations - the answer splits**: the bounded state **beats retrieval decisively (+0.292, CI [+0.146, +0.438], 16W/2L)** and **regresses against the full transcript (-0.250, CI [-0.417, -0.063], 5W/17L)**, and the mechanism is measured: the views carry a median of 44, 6 and 1 prior actions respectively (`g2d-view-composition-v1.json`) | **half met: better than raw retrieval, not yet no-regression** |
| G4 winning region wider than 4/48, not only the 2K extreme | material widening | 4/48 -> **157 of 724** cells at a quality-admissible state (77 at the 4,096-token floor, 18 at 6,144, 59 at 8,192, 3 at 16,384), across **all five regimes** the replay models - balanced, slow-worker hotspot, worker loss, a flash crowd and a heavy-tailed size mix - with 117 clearing the 1.5x goodput bar.  **And on a workload whose sessions actually reach a million tokens, which the main grid's 8-turn sessions never did (realised max 113,300, now recorded per receipt), the region advances in 16 of 20 cells and every one clears the 30% p99 bar**, in four of the five regimes at both 4,096 and 8,192 | **met** (the widening came from the grid's missing axes - active-set granularity, session scale, model geometry, capacity pressure, workload shape - not from the compiler) |
| cluster consequence >=1.5x goodput or >=30% p99 in several regimes | two or more regimes | both routes clear, in different regimes, with the tail's scope measured rather than assumed: capacity-pressure and worker-loss cells advance on goodput (median 1.61x, max 5.06x over 117 cells) while the flash crowd advances on the tail (23 cells at >=30% p99, best +68.7%) - and sweeping the burst's own parameters (`g4b-burst-sensitivity-v1.json`) shows that tail result holds in **6 of 9 burst shapes at the 4,096-token state** (advancing in the same 6) and **1 of 9 at 8,192**, where several settings have a negative p99 reduction, so it is **conditional on the workload and on the state size**, not universal; balanced with all sessions resident and a KV-moving tier still does not advance | **met on goodput everywhere, and on the tail at the size the system runs at** |
...
| C2b | at a fixed budget the window and the decision trade off, measured along the window axis | Three window points at a fixed 8,192-token budget, all at fidelity **0.00 pp**: 6,656-token window + ~1.5K retrieval, decision **0.089** (the full-history level); 4,096 + ~4.1K, **0.161**; 2,867 + ~5.3K with a compiled far field, **0.179**.  Paired on shared instances every step towards the smaller window is positive and none is resolvable alone: 6,656 -> 4,096 **+0.072, CI [-0.041, +0.188]** (8W/5L, n=48); 6,656 -> 2,867 **+0.069, CI [-0.121, +0.271]** (5W/5L, n=24); 4,096 -> 2,867 **+0.040, CI [-0.132, +0.220]** (4W/4L, n=24).  No window at all: fidelity -22.84 pp (dedup + snippet at a 4,096-token budget), decision 0.243 (n=48).  The window's *surface* requirement is therefore ~2.9K, and every token the surface does not need is a token the decision does not get | n=24-48 per pair: the pairwise steps are smaller than the resolvable difference at this sample size even though the three-point direction is consistent; 2,867 is n=24, so the low end of the axis is the least sampled |
| C2 | fidelity parity needs the newest **~3.5K** kept whole **and a far field that is not prefix-truncated raw dumps**; the total can be as small as **4,096 tokens** when the window is 3,584 (fidelity **-0.96 pp**, inside the allowance), while the identical total with a 3,072-token window scores -2.40 pp - the window, not the budget, is what carries the surface | window sweep 2.9K-6.1K with a consolidated far field -> 0.00 pp at both 8,192 and 12,288 budgets; the same 3,072-token window with *raw* retrieval behind it scores -6.94 pp at 8,192 and -6.07 pp at 12,288 (the truncated dumps in the far field hurt the surface even though the newest evidence is whole, and more budget does not fix it) | teacher-forced metric, whose recency saturation is itself C10; measured on 44 long-history examples.  **The budget floor is measured at 6,144 tokens**: the same 3,072-token window scores 0.00 pp and a decision of 0.150 (tied with retrieval at 4,096: +0.093, CI [-0.030, +0.217], 13W/9L) at 6,144, while at 4,096 it scores -2.40 pp (outside tolerance) with the decision down to 0.117 against 0.243 for plain retrieval at that total - so 6,144 is the smallest budget at which both hold, measured from below and above |
...
| C4b | is model-written compaction worse than window + retrieval? | **No - they are equivalent on this corpus.**  Same budget (8,192), same window (6,656), remainder 0.5K summary vs 1.5K retrieval: fidelity 0.00 pp both ways, decision 0.131 vs 0.089, paired **+0.042, 95% CI [-0.031, +0.117]** (8W/3L); against the shipped 4,096-window view, **-0.030, CI [-0.147, +0.083]** (8W/8L).  What the table does show is that the *window/budget split* is the lever: 0.089-0.131 at a 6,656 window against 0.161 at 4,096, same fidelity.  The earlier row that reported compaction as catastrophic (-23.86 pp) is withdrawn and was a bug, not a result | 48 paired sessions, 26 scoreable: 0.03-0.07 differences are not resolvable at that size.  Scope: one summariser family, one split.  Both compaction receipts record the configuration that produced them (window 6,656, summary 512) and their summariser call counts, and the corrected runner reproduces the fidelity arm's rows exactly |
...
| C5 | mobility law and inversion | 1M with a 4,096 state: 0.0954 s; with 8,192: 0.2031 s; against 0.4855 s for 32K carrying the 16,384-token pre-compiler view; flat 0.2028-0.2031 s from 32K to 1M | composed from G3 measured primitives; the lookup term is measured on composed million-token histories (0.4-4.4 ms p50, query-dependent, `g2-index-lookup-composed-1m-v1.json`) rather than extrapolated |
...
| C7 | the cost law moves routing at a quality-admissible state, including a hotspot | **174/724** replay cells advance, **157 of them at a fidelity-admissible state**: **77 at 4,096** (the measured floor, -0.96 pp with a 3,584-token window), **18 at 6,144** (0.00 pp), 59 at 8,192 (0.00 pp) and 3 at 16,384 (-1.94 pp, retrieval-only), across **all five regimes** the replay models: balanced, slow-worker, worker-loss, a **flash crowd** (half the sessions arriving inside a 4 s window) and a **heavy-tailed size mix** (Zipf(1.2), so most sessions are 32K and a few 262K/1M).  A hotspot variant (one worker at a tenth of the service rate) advances in all three worker-behaviour regimes (16/48 cells).  The magnitudes across the whole admissible region: all **157** cells clear the **1.5x SLO-goodput** bar that defines an advance, 117 of them with a finite ratio (median **1.61x**, max **5.06x**) and 40 against a baseline that completes *no* work, so those ratios are unbounded rather than large.  **23 cells also clear the 30% p99 bar, all of them in the flash-crowd regime** (8 at 4,096, 8 at 6,144, 6 at 8,192, 1 at 16,384; best **+68.7%**): under a fleet-wide burst a baseline that must move or re-prefill the history queues behind the spike while 4-8K of rematerialisation does not, so the tail is where the changed cost law shows up most sharply.  Outside that regime it is a throughput win: where the baseline stalls, the ephemeral policy's own p99 is worse in absolute terms | the balanced, hotspot and slow-worker wins are the no-cluster-KV-store regime with a warm cache holding a fraction of the fleet.  **And the decision at the 8,192 state is a statistical tie with retrieval, not a point-estimate win**: the joint arm scores 0.161 against plain retrieval's 0.237 floor, so under a strict point-estimate parity bar **0** cells clear it, while the paired interval (+0.081, CI [-0.016, +0.177]) includes zero.  A phase-diagram cell can be called quality-admissible for the surface, or for the decision against full history, but not for beating retrieval |
...
| C11 | what would widen the quality-admissible routing region | **Done, and it was one measurement**: the state is admissible at **4,096 tokens** (-0.96 pp with a 3,584-token window), which is exactly the size of the replay's 4,096 column (**63 of 198** cells against the 8,192 column's 52 of 294).  The join now reads **117 fidelity-admissible cells** at half the previous state size.  The decision at this size is still read from the 3,072-window arm (0.117) until the 3,584-window decision arm lands; the strict parity bar (0.237-0.243) clears none of them either way, with the deficit in the far field rather than the window (the same 3,072-token window holds 0.00 pp once the total is 6,144).  So a 4,096-token state inside the allowance would widen the region from **52 to 115 cells at half the state size**, with capacity and mobility improving with it - the state halving and the region widening are the same measurement.  The stricter bar, decision parity with plain retrieval, needs 0.237-0.243 F1 against the 0.161-0.179 that the fidelity-admissible arms reach: a gap of 0.06-0.08 that no arm we have measured closes | `g2-compiler-window3k-b{4096,6144}-v1.json`, `g4-quality-join-v2.json`.  The 4,096-token admissibility is a *target*: it is the run the queue answers, and it is stated as a target rather than a result |
...
the explanation everyone reaches for first.  The window-size isolation runs, the equal-budget
retrieval reference, the dedup-only arm and the killer table's long-history column are all measured
and in the ledgers.

What would move it further is stated as two numbers rather than as hope (C11).  The 4,096-token
state is **0.4 pp** of fidelity away from the allowance, and its column in the replay advances
**63** cells against the admissible 8,192 column's **52** - so halving the state and widening the
region from 52 to 115 cells are the same measurement, and the far field rather than the window is
where the 0.4 pp is.  The stricter bar, decision parity with plain retrieval, needs **0.06-0.08**
F1 that no arm measured here produces.  If a compiler cannot move the first number, the paper's
claims stand as they are - the bound, the flat law, the inversion, capacity, recovery and the
region at 8,192 - and Best is not claimed on the strength of a smaller state that was not
measured.
