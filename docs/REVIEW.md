...
| 8 | "The state bound is asserted, not derived - why 4-8K and not less?" | Because it is measured from below and above: 4,096 with a 3,072-token window fails the fidelity allowance (-2.40 pp), the same 4,096-token total with a 3,584-token window holds it (-0.96 pp), and 12,288 does not improve either metric. The bound is also a *design choice* rather than an intrinsic ceiling - lexical evidence mass grows with session age on this corpus, and the paper says so. | `g2-compiler-window3k-b*.json`; `g2-evidence-mass-v2.json` |
...
| 10 | "You shipped a wrong number once - the compaction baseline was a window-only view.  Why trust the rest?" | Because the error was found by a check, not by luck, and the class it belongs to is now asserted everywhere.  The semantic audit used to ask whether an arm *called* its mechanism; it now asks whether the mechanism's output reached the scored context (`summary_tokens_in_view`, asserted per row for both compaction arms), which is the fact whose absence produced the error.  The same discipline produced the other corrections in this paper - a target-rebinding bug, a snippet budget that let a view exceed its own bound, a runner whose window had drifted from its receipt, a paired test whose key was silently missing - each of them found by an audit, each of them recorded in Appendix A.2 rather than quietly fixed. | `check_receipts.py`, `check_scripts.py`, `paper_numbers.py`; §A.2 |
| 11 | "Your bounded view gives up a few points to a resident one at long histories, so the bound costs quality." | It does, and the paper says so with numbers rather than adjectives: against a resident view that uses the model's whole 131,072-token window, the state is 0.38 pp behind at 145K-token histories, 3.26 pp at 278K, 4.91 pp at 514K and 3.35 pp at 984K (paired medians, per-example spread -26 to +2 pp), while the end task does not degrade at all (0.077 -> 0.122 -> 0.167).  That is the trade the abstraction makes: a bounded state that costs a few points of surface against the largest view a machine can hold, in exchange for placement, capacity and recovery that stop depending on age. | Table 7b; §6 |
...
| "157 cells at a quality-admissible state - 77 at the 4,096-token floor, 18 at 6,144, 59 at 8,192 and 3 at 16,384, across balanced, slow-worker, worker-loss, a flash crowd and a heavy-tailed size mix - every one of them clearing the 1.5x goodput bar, 23 of them the 30% p99 bar" | `g4-quality-join-v3.json`, `g4-all-phase-summary-v3.json`, `g4b-burst-*-v1.json`, `g2-compiler-window3584-b4096-v1.json` |
