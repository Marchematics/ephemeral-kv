...
# the window/budget split the compaction baseline is compared against: the same 6,656-token window
# as `run_compaction.sh`, with the remainder spent on consolidated retrieval instead of a summary
# (this is the row that says the two designs differ only in what the remaining ~1.5K buys).  The
# decision arm's receipt records this configuration itself.
/root/qcc/venv/bin/python benchmarks/g2b_patch_localization.py \
  --jsonl data/sessions-patch-long.jsonl --model /root/qcc/models/Llama-3.2-1B-Instruct \
  --max-length 65536 --max-new 96 --max-examples 48 "${g2b[@]}" \
  --compile-mode tail_state --tail-tokens 6656 \
  --out artifacts/g2b-patch-localization-window6656-b8192-n48.json
/root/qcc/venv/bin/python benchmarks/g2_model_quality.py --jsonl data/sessions-64k.jsonl \
  --model /root/qcc/models/Llama-3.2-1B-Instruct --max-length 131072 --max-examples 48 \
  "${g2[@]}" --compile-mode tail_state --tail-tokens 6656 \
  --out artifacts/g2-compiler-window6656-b8192-v1.json
