#!/bin/bash
# The column the region has been missing: 6,144 tokens.
#
# The paper's measured state floor is 6,144 tokens (4,096 scores -2.40 pp of fidelity, 6,144 scores
...
#   1. the warm-prefill cost at 6,144 tokens, measured like the others (the replay's costs must stay
#      measurements, not interpolations);
#   2. the grid cells at that active size, replayed over the same regimes as the capacity grid.
#
# The quality point is *not* re-measured: `g2-compiler-window3k-b6144-v1.json` and
# `g2b-patch-localization-window3k-b6144-n48.json` already carry it.
set -x
cd "$(dirname "$0")/.." || exit 1
...
common=(--g3 artifacts/g3-hardware-primitives-qwen05b-v1.json
        ...
        --sessions 16 --workers 4 --active-tokens 6144)
  ...
  for tier in 0 8 32; do
    for warm in 4 16; do
      /root/qcc/venv/bin/python benchmarks/g4_routing_replay.py "${common[@]}" \
        ...
        --out "artifacts/g4b-capacity-h${tag}-warm${warm}-tier${tier}-a6144-v1.json"
    done
  done
done

/root/qcc/venv/bin/python benchmarks/g4_phase_summary.py --glob 'artifacts/g4b-capacity-*-a6144-v1.json' \
  --out artifacts/g4b-capacity-a6144-phase-summary-v1.json
